...
| 固件 | 参考方式 | 主要用途 |
|---|---|---|
| `ESP32C3_ADS1299_SRB1_BLE` | 固定 SRB1 | 测量电极接 `INxP`，公共参考接 SRB1，支持 USB 和可靠 BLE |
| `ESP32C3_ADS1299_SRB2_BLE` | 默认 SRB2，可切换 SRB1 | 测量电极接 `INxN`，公共参考接 SRB2，支持运行时切换参考方式 |
| `ESP32C3_ADS1299_SRB1_PONLY_REFERENCE` | 固定 SRB1 | SRB1 P-only 参考实现，用于有线采集和配置基准 |
| `ESP32C3_ADS1299_SERIAL_STABLE` | 串口方案 | 不使用 BLE 时的稳定 USB 串口采集版本 |

长时间 BLE 采集应使用仓库中的 V18 SRB1 或 SRB2 BLE 固件。旧版可靠传输固件可以连接 V18 GUI，但不具备完整的采集/发送任务隔离和过期 NACK 抑制。
...
| 命令 | 功能 |
|---|---|
| `A5` | 一次性下发参考模式、启用掩码、BIAS 掩码、SRB2 掩码和 8 通道增益 |
| `A6` | 写入指定 ADS1299 寄存器，当前主要用于修改 BIAS mask |
| `A7` | 配置单个通道的开关、PGA、BIAS 和 SRB2 标志 |
| `A8` | 在支持的固件上切换 SRB1/SRB2 参考模式 |
| `A9` | 设置 lead-off 检测通道掩码 |

配置完成后，固件通过 STATUS 特征或串口返回寄存器读回结果。上位机只有在读回值与请求一致时才确认配置成功。

SRB1 固定版本会忽略 `A7` 中的 SRB2 标志，并保持所有 `CHnSET.SRB2=0`。可切换版本会根据当前参考模式设置 `MISC1.SRB1`、逐通道 SRB2 位和对应的 BIAS 取样侧。

### 4.8 SRB1 与 SRB2 差异

| 项目 | SRB1 模式 | SRB2 模式 |
|---|---|---|
| 测量电极 | `IN1P`～`IN8P` | `IN1N`～`IN8N` |
| 公共参考 | SRB1 | SRB2 |
| ADS 原始极性 | `INxP - SRB1` | `SRB2 - INxN` |
| `MISC1.SRB1` | 正常 EEG 时开启 | 关闭 |
| `CHnSET.SRB2` | 关闭 | 对启用通道开启 |
| 推荐 BIAS 侧 | `BIAS_SENSP` | `BIAS_SENSN` |

固件不会为了统一显示方向而翻转原始数据。上位机和后续算法应结合实际参考方式解释极性。
...
BLE STATUS V4 使用独立的 72 字节状态包，配置 ACK 和诊断信息不插入 EEG DATA 流。状态内容包括：
