...
- Late insertion failed on 19.5.0 but card-in-place reset worked. 19.5.1 forces disk deinitialization after failed mount to clear retained HAL error state. Nine tool tests and signed build/key verification pass.
- 19.5.1 was flashed and verified on 2026-09-07. After booting without a card, inserting the FAT32 card without reset opened `BCI00009.BIN`; acquisition and RF continued while SD mount/record became active with zero reported SD errors or drops.
- Dongle stop plus the `AB TOKEN` query returned stopped/file-closed/no-error. Readback of `BCI00008` and the late-inserted `BCI00009` found zero bad headers, CRC failures, sequence breaks or tail bytes. Both MET files contain matching start/progress/end records; final `synced_bytes` and `last_seq` match each BIN exactly.
- Pending: SD write-failure and power-loss injection, calibrated sampling-rate checks and physical trigger verification. UTC synchronization and PC MET import remain out of scope. No full power-loss safety guarantee.
