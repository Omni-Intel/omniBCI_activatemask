/*
  ESP32-C3 + ADS1299：独立全差分固件 V20.0（软件 SPI）
  ...
  本固件目标是把采集链路状态完整、可验证地呈现出来：
    ...
    5. 每路固定采集 INxP-INxN，SRB1/SRB2 永久关闭；
    6. 保留短路噪声和内部方波诊断模式；
    7. 数据流中绝不插入状态文字，避免周期 impulse 和解析错位。

  默认上电配置：CH1-CH8 全开，每路 CHnSET.SRB2=0，MISC1.SRB1=0，
  BIAS 与 SRB 解耦，SENSP=0xFF, SENSN=0xFF。
  ...
  串口命令：
    b : 开始二进制数据流
    s : 停止数据流
    e/p/m/* : 全差分推荐模式，BIAS 同时取 P/N（CH1-8）
    n : 兼容命令，同样保持全差分并让 BIAS 同时取 P/N（CH1-8）
    o : 全差分采集，关闭 BIAS，用于判断 BIAS 环路是否引入问题
    q : 所有通道输入内部短路，用于测板级底噪
    t : 所有通道接 ADS1299 内部方波，用于验证 SPI/数字链路
    1 / 2 / 4 / 6 / 8 / 12 / 24 : 修改 ADS1299 PGA 增益并重配当前模式
    r : 清零诊断计数
    ? : 停止数据流后打印诊断信息和寄存器读回
    0xAB : 返回固件版本、协议、能力位和 full-differential profile
...
/*
  Independent full-differential variant

  Wiring:
    channel 1 pair -> IN1P and IN1N
    ...
    channel 8 pair -> IN8P and IN8N
    bias electrode -> BIASOUT

  Enforced configuration:
    - Every enabled channel measures its own INxP-INxN pair.
    - CHnSET.SRB2 is always 0, including normal, shorted and test modes.
    - MISC1.SRB1 is always 0.
    - BIAS is independent of SRB; the default mode applies the logical BIAS
      mask to both BIAS_SENSP and BIAS_SENSN.
*/
...
// Two seconds of capture elasticity without taking so much heap that the
// Bluedroid stack can fail to initialize. This is only a shock absorber; BLE
// TX flow control below is what prevents a sustained backlog.
...
constexpr uint8_t FIRMWARE_VERSION_MAJOR = 20;
...
constexpr uint16_t DEVICE_CAPABILITIES = 0x006Fu;
constexpr uint8_t FIRMWARE_PROFILE_FULL_DIFF = 0x03u;

// ============================ BLE reliable transport STATUS V5 / DATA V2 ============================
constexpr char BLE_DEVICE_NAME[] = "OmniBCI-C3-FULLDIFF-V20";
...
// V20 keeps configurationGeneration at byte 72 and appends acquisition-side
// diagnostics, allowing the host to distinguish DRDY loss from BLE loss.
...
enum FrontendMode : uint8_t {
  MODE_EEG_BIAS_PN       = 0,  // legacy protocol value; electrically full differential
  MODE_FULL_DIFF_BIAS_PN = 1,  // default: BIAS_SENSP/SENSN use the same mask
  MODE_EEG_BIAS_OFF    = 2,
...
volatile FrontendMode currentMode = MODE_FULL_DIFF_BIAS_PN;
...
// Arduino-ESP32 Bluedroid reports whether notify() was accepted by the host
// stack through onStatus() before notify() returns. Preserve that result so a
// congested submission is retried instead of being falsely marked as sent.
...
// GUI 可动态修改的逻辑 BIAS mask；正常模式同时写入 BIAS_SENSP/SENSN。
volatile uint8_t currentBiasMask = ADS_ACTIVE_CH_MASK;
volatile uint8_t currentLeadOffMask = 0x00;

// 二进制控制协议：0xA6 <register> <value>
// Python/MATLAB GUI 用 0xA6 0x0D mask 来运行时修改逻辑 BIAS mask。
...
// A7 channel is 0..7; flags: bit0 enabled, bit1 joins both BIAS_SENSP/SENSN.
// bit2 is accepted for wire compatibility but ignored; SRB2 always remains off.
static uint8_t binaryControlRegister = 0;
static uint8_t binaryControlChannel = 0;
static uint8_t binaryControlGain = 24;
// A5 bulk config payload: legacy reference, enabled mask, BIAS mask,
// legacy SRB2 mask, gains[8]. Reference/SRB2 fields are ignored.
...
void setup() {
  ...
  startNscClock();
  hardwareResetAds();
  configureFrontendLocked(MODE_FULL_DIFF_BIAS_PN);
  ...
  Serial.println("ESP32C3 ADS1299 FULL DIFFERENTIAL FIRMWARE V20.0 / PROTOCOL V1 READY");
  ...
  Serial.println("Commands over USB or BLE CONTROL: b/s/e/p/m/*/n/o/q/t/1/2/4/6/8/12/24/r/? plus binary A6/A7/A9/AA/AB");
}
...
void transportTask(void *argument) {
  ...
  for (;;) {
    ...
    if (bleInitialized &&
      ...
      // One 92-byte packet every 30 s is negligible beside EEG DATA and is
      // essential when the link is unhealthy: suppressing STATUS exactly when
      // in-flight occupancy was high hid ring overflow/DRDY evidence from logs.
      if (bleConnected) {
...
void processSerialByte(char c) {
  ...
  if (byteValue == 0xAB) {
    flushNumericCommand();
    sendConfigAck(0xAB, FIRMWARE_PROFILE_FULL_DIFF);
    return;
  }
...
void handleCommand(char command) {
  switch (command) {
    ...
    case 'M':
    case '*':
      // 推荐全差分配置：每路 INxP-INxN，BIAS 同时取 P/N。
      configureFrontend(MODE_FULL_DIFF_BIAS_PN);
      return;
...
// CHnSET bit 3 is deliberately absent, so the shared negative-input bus stays off.
uint8_t makeChannelSetting(uint8_t gainCode, uint8_t mux) {
...
void setBiasMask(uint8_t mask) {
  ...
  // the whole front end so BIAS_SENSP/SENSN remain consistent with mode.
  currentBiasMask = static_cast<uint8_t>(mask & currentEnabledMask);
...
void setChannelConfig(uint8_t channel, uint8_t gain, uint8_t flags) {
  ...
  if ((flags & 0x02u) && (flags & 0x01u)) currentBiasMask |= bit;
  else currentBiasMask &= static_cast<uint8_t>(~bit);
  configureFrontend(static_cast<FrontendMode>(currentMode));
  if (!streamingEnabled) {
    Serial.printf("CH%u config: %s PGA=%ux BIAS_PN=%u SRB1=OFF SRB2=OFF\n",
...
bool setBulkChannelConfig(const uint8_t *payload) {
  ...
  currentBiasMask = static_cast<uint8_t>(payload[2] & currentEnabledMask);
  for (uint8_t ch = 0; ch < 8; ++ch) {
  ...
  // Full-differential firmware ignores both legacy reference fields and
  // performs one complete write/readback cycle with SRB1/SRB2 forced off.
  configureFrontend(static_cast<FrontendMode>(currentMode));
...
void configureFrontendLocked(FrontendMode mode) {
  ...
  uint8_t biasP = currentBiasMask;
  uint8_t biasN = currentBiasMask;

  switch (mode) {
    case MODE_EEG_BIAS_PN:
    case MODE_FULL_DIFF_BIAS_PN:
      config2 = CONFIG2_NORMAL;
      biasP = currentBiasMask;
      biasN = currentBiasMask;
      break;
  ...
  writeAdsRegister(0x0F, leadOffMask);  // LOFF_SENSP: test every selected P input
  writeAdsRegister(0x10, leadOffMask);  // LOFF_SENSN: test every selected N input
  writeAdsRegister(0x11, 0x00);         // LOFF_FLIP
  writeAdsRegister(0x15, MISC1_SRB1_OFF);
...
bool verifyFrontendLocked(FrontendMode mode) {
  ...
  uint8_t expectedBiasP = currentBiasMask;
  uint8_t expectedBiasN = currentBiasMask;
  ...
  for (uint8_t address = ADS_FIRST_CH_REG; address <= ADS_LAST_CH_REG; address++) {
    ...
    const uint8_t observed = readAdsRegister(address);
    ok &= observed == (enabled ? expectedActive : expectedDisabled);
    ok &= (observed & 0x08u) == 0x00u;  // CHnSET.SRB2 must never become set.
  }
  ...
  ok &= readAdsRegister(0x10) == expectedLeadOffMask;
  ok &= readAdsRegister(0x11) == 0x00;
  ok &= readAdsRegister(0x15) == MISC1_SRB1_OFF;
  return ok;
}
...
bool sendBleBytes(const uint8_t *data, size_t length) {
  ...
  // DATA V2 is designed to fit one negotiated MTU. Splitting here would
  // destroy packet boundaries at the receiver.
...
void serviceReliableBleTx() {
  ...
  if (bleReliableNackPending) {
    ...
    if (block) {
      transmitted = sendReliableBlock(*block, true);
      retryRequired = !transmitted;
    } else {
      ...
      if (alreadyAcked || notProducedYet) {
        ...
        transmitted = sendReliableGapMarker(requested);
        retryRequired = !transmitted;
        if (transmitted) bleReliableLastTxMs = millis();
      }
    }
    // A congested notify was not transmitted. Keep the NACK cursor on the
    // same block so the next service turn retries it after backoff.
  ...
  while (reliableSeqLessOrEqual(bleReliableNextNewTxSequence, windowLast) &&
    ...
    // Only advance after Bluedroid accepted the notification. Previously an
    // ESP_ERR congestion result permanently skipped this unsent block.
    if (sendReliableBlock(*block, false)) bleReliableNextNewTxSequence++;
    return;
  }
}
...
void buildStreamFrame(
  ...
  if (currentMode == MODE_EEG_BIAS_PN || currentMode == MODE_FULL_DIFF_BIAS_PN) {
    flags |= (1u << 6);  // BIAS derives from both P and N inputs.
  }
  // Bit 7 remains clear: MISC1.SRB1 is permanently off in V20 full differential.
  p[15] = flags;
...
bool readAdsFrame(uint8_t *destination, bool &drdyWasLow) {
  ...
  // Do not mask interrupts for the complete 216-bit software-SPI frame.
  // It takes roughly 2.9 ms every 4 ms, which starves the ESP32 BLE stack and
  // causes notify congestion plus frameQueue overflow. ADS1299 SCLK may pause
  // while CS stays asserted, so an ISR can safely stretch a clock phase.
  selectAds();
  delayMicroseconds(1);
...
void handleDeviceControl(const DeviceControlCommand &command) {
  if (command.type == DEVICE_MSG_HELLO) {
    uint8_t payload[11] = {
      ...
      static_cast<uint8_t>(DEVICE_CAPABILITIES & 0xFFu),
...
void sendConfigAck(uint8_t command, uint8_t argument) {
  ...
  if (command == 0xAB) {
    reply[3] = FIRMWARE_VERSION_MAJOR;
    reply[4] = FIRMWARE_VERSION_MINOR;
    reply[5] = FIRMWARE_VERSION_PATCH;
    reply[6] = DEVICE_PROTOCOL_VERSION;
    reply[7] = static_cast<uint8_t>(DEVICE_CAPABILITIES & 0xFFu);
    reply[8] = static_cast<uint8_t>((DEVICE_CAPABILITIES >> 8) & 0xFFu);
    reply[9] = configurationVerified ? 0x01u : 0x00u;
  } else if (runPhase != PHASE_STREAMING) {
    xSemaphoreTake(adsBusMutex, portMAX_DELAY);
    if (command == 0xAA) {
  ...
  // V20 BLE configuration responses use the independent RESPONSE characteristic.
...
void printHelpAndDiagnostics() {
  Serial.println();
  Serial.println("=== ADS1299 V20 independent full-differential diagnostic ===");
  ...
  Serial.printf("enabledMask=0x%02X biasLogicalMask=0x%02X SRB1=OFF SRB2=OFF\n",
                currentEnabledMask, currentBiasMask);
  for (uint8_t ch = 0; ch < 8; ++ch) {
    Serial.printf("CH%u: %s PGA=%ux BIAS_PN=%u SRB1=0 SRB2=0\n",
      (unsigned)(ch + 1), (currentEnabledMask & (1u << ch)) ? "ON" : "OFF",
      (unsigned)channelGain[ch],
      (unsigned)((currentBiasMask >> ch) & 1u));
  }
  Serial.println("commands: b s e/p/m/* n o q t 1/2/4/6/8/12/24 r ? binary AB=version");
...
void printRegisterReadback() {
  ...
  uint8_t channelRegisters[8] = {};
  for (uint8_t ch = 0; ch < 8; ++ch) {
    channelRegisters[ch] = readAdsRegister(static_cast<uint8_t>(ADS_FIRST_CH_REG + ch));
  }
  const uint8_t biasP = readAdsRegister(0x0D);
  const uint8_t biasN = readAdsRegister(0x0E);
  const uint8_t leadOffP = readAdsRegister(0x0F);
  const uint8_t leadOffN = readAdsRegister(0x10);
  ...
  for (uint8_t ch = 0; ch < 8; ++ch) {
    Serial.printf("CH%uSET=0x%02X SRB2=%u\n",
      (unsigned)(ch + 1), channelRegisters[ch],
      (unsigned)((channelRegisters[ch] >> 3) & 1u));
  }
  Serial.printf("BIAS_SENSP=0x%02X BIAS_SENSN=0x%02X MISC1=0x%02X\n", biasP, biasN, misc1);
  Serial.printf("LOFF_SENSP=0x%02X LOFF_SENSN=0x%02X SRB1=%u\n",
    leadOffP, leadOffN, (unsigned)((misc1 >> 5) & 1u));
}
