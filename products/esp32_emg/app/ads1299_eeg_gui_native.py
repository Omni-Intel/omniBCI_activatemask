...
Reliable BLE V18 long-run continuity transport with asynchronous one-minute
BIN segmentation. This module hosts the MainWindow orchestrator and the
application entry point; the protocol parsers, ring buffers, live filter,
segmented BIN writer and transport workers live in the ``omnibci`` package.
"""
...
# This module intentionally re-exports the moved symbols: the test-suite and
# colleague scripts import them from ``ads1299_eeg_gui_native`` directly, and
# ``mock.patch.object(gui, "decode_config_snapshot")`` style patches rely on
# the names staying in this namespace.
from omnibci.constants import *  # noqa: F401,F403
from omnibci.filtering import FilteredBatch, LiveFilterWorker  # noqa: F401
from omnibci.protocol import (  # noqa: F401
    AdsFrameParser,
    Frame,
    crc16_ccitt,
...
from omnibci.recording import AsyncRawWriter  # noqa: F401
from omnibci.ring_buffer import RingBuffer  # noqa: F401
from omnibci.transport_ble import (  # noqa: F401
    BLE_AVAILABLE,
    BLE_IMPORT_ERROR,
    BleTransportWorker,
)
from omnibci.transport_serial import SerialTransportWorker  # noqa: F401
from omnibci.ui_widgets import (  # noqa: F401
    ClockAxisItem,
    PsdWorker,
    PsdWorkerSignals,
    SoftTriggerWindow,
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.app_settings = QtCore.QSettings("OmniBCI", "ESP32_EMG_V20")
        self.setWindowTitle("OmniEMG Full Diff | software V20 integration.1 | ADS1299 firmware V20")
        ...
        self.ring = RingBuffer(CHANNELS, self.sample_rate * 90)            # untouched input-referred uV
        self.filtered_ring = RingBuffer(CHANNELS, self.sample_rate * 90)   # continuous causal display chain
        ...
        self.display_target_delay_samples = int(round(DISPLAY_JITTER_BASE_TARGET_S * self.sample_rate))
        self.display_startup_samples = int(round(DISPLAY_JITTER_STARTUP_S * self.sample_rate))
        self.display_min_reserve_samples = max(1, int(round(DISPLAY_JITTER_MIN_RESERVE_S * self.sample_rate)))
        ...
        self.sos_display_band = signal.butter(2, [5.0, 50.0], btype="bandpass", fs=self.sample_rate, output="sos")
        notch_sections = []
        for notch_hz in (50.0, 100.0):
            notch_b, notch_a = signal.iirnotch(notch_hz, 30.0, fs=self.sample_rate)
        ...
        self.stream_server = LocalStreamServer(
            port=8766, sample_rate=self.sample_rate,
    ...
    def _build_ui(self):
        ...
        self.psd_max_spin.setRange(20, self.sample_rate / 2)
    ...
    def _build_omni_ui(self):
        ...
        rate_toolbar = self.addToolBar("EMG sample rate")
        rate_toolbar.setMovable(False)
        ...
        self._add_tool_field(rate_toolbar, "采样率", self.sample_rate_combo)
        self.sample_rate_apply_btn = QtWidgets.QToolButton()
        self.sample_rate_apply_btn.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_DialogApplyButton))
        self.sample_rate_apply_btn.setToolTip("应用采样率（仅停止采集时）")
        self.sample_rate_apply_btn.clicked.connect(self.apply_sample_rate)
        rate_toolbar.addWidget(self.sample_rate_apply_btn)
        ...
        self.reference_combo.setToolTip(
            "V20 全差分：每路独立 INxP-INxN；SRB1/SRB2 OFF。"
        )
        ...
        self.psd_raw_check.stateChanged.connect(self.reset_psd_smoothing)
        self.psd_max_spin = QtWidgets.QDoubleSpinBox()
        self.psd_max_spin.setRange(10.0, self.sample_rate / 2)
    ...
    def _filter_settings_changed(self, *_):
        ...
        if hp >= lp:
            return
        self.sos_display_band = signal.butter(2, [hp, lp], btype="bandpass", fs=self.sample_rate, output="sos")
        self.reset_processing_state()
    ...
    def _start_time_changed(self, value):
        if self.offline_uv is not None:
            self.offline_end = min(self._total_samples(), int((value + self.win_spin.value()) * self.sample_rate))
    ...
    def _window_changed(self, *_):
        if self.offline_uv is not None:
            self.offline_end = min(
                self._total_samples(),
                int((self.start_time_spin.value() + self.win_spin.value()) * self.sample_rate),
            )
            self.reset_psd_smoothing()
        self.update_fast_plots()
    ...
    def _nav_region_changed(self, region=None):
        ...
        if self.offline_uv is not None:
            self.offline_end = min(self._total_samples(), int(hi * self.sample_rate))
            self.reset_psd_smoothing()
        self.update_fast_plots()

    def page(self, direction):
        total = self._total_samples() / self.sample_rate
    ...
    def impedance_series_default_kohm(self) -> float:
        # ponytail: no EEG resistor assumption; calibrate the full P/N loop externally.
        return 0.0
    ...
    def set_reference_mode_local(self, mode: int):
        self.reference_mode = REFERENCE_SRB1  # V20's reserved control byte is zero, not a topology selector.
    ...
    def _decode_config_ack_packet(
        ...
        return {
            "packet": packet,
            "config1": packet[3],
            "sample_rate_hz": int.from_bytes(packet[4:6], "little"),
    ...
    def open_channel_settings(self, ch):
        ...
        note_text = (
            "V20 全差分：每路独立 INxP-INxN，"
            "SRB1/SRB2 OFF；BIAS 同时作用于 P/N。"
        )
    ...
    def apply_reference_mode(self):
        """V20 has no selectable reference topology."""
        self.set_reference_mode_local(REFERENCE_SRB1)
        self.set_status("V20 全差分：INxP-INxN；SRB1/SRB2 OFF。")

    def set_sample_rate_local(self, rate: int):
        sample_rate_code(rate)
        if self.streaming and rate != self.sample_rate:
            raise RuntimeError("Stop acquisition before changing sample rate")
        if rate == self.sample_rate:
            return
        # Finish PSD work before changing the rate its bound calculation uses.
        self.psd_pool.waitForDone()
        ...
        self.lp_spin.setMaximum(min(450, rate / 2 - 1))
        self.lp_spin.blockSignals(False)
        self.psd_max_spin.setMaximum(rate / 2)
        self.sos_display_band = signal.butter(
            2, [self.hp_spin.value(), self.lp_spin.value()],
            btype="bandpass", fs=rate, output="sos")
        self.sos_notch = np.vstack([
            signal.tf2sos(*signal.iirnotch(hz, 30.0, fs=rate))
            for hz in (50.0, 100.0)])
        self.display_min_reserve_samples = max(1, int(round(DISPLAY_JITTER_MIN_RESERVE_S * rate)))
        self.reset_processing_state()
        self.reset_live_session_metrics()
        self.reset_display_jitter_buffer()
        self.sample_rate_combo.setCurrentIndex(SAMPLE_RATES.index(rate))
        self.impedance_btn.setToolTip("ADS1299: 6 nA, nominal 31.25 Hz; P/N loop calibration required")
        if self.stream_server is not None:
            self.stream_server.sample_rate = rate

    def configure_sample_rate(self, rate: int):
        code = sample_rate_code(rate)
        if self.streaming or self.impedance_active or self.raw_recording_enabled:
            raise RuntimeError("Stop acquisition/impedance before changing sample rate")
        if not self.transport_connected():
            raise RuntimeError("Connect the V20 full-differential device first")
        self.hardware_sample_rate = None  # Lost replies leave hardware state unknown; block acquisition.
        if self.active_transport == "ble":
            snapshot = decode_config_snapshot(self.ble_worker.request_blocking(
                MSG_SET_SAMPLE_RATE, bytes((code,)), timeout=4.0))
            if snapshot.sample_rate_hz != rate:
            ...
            ack = self.read_config_ack(0xAA, expected_argument=code, timeout=2.0)
            if (ack is None or not ack["verified"] or ack["sample_rate_hz"] != rate
                    or ack["sample_rate_code"] != code or ack["config1"] != 0x96 - code):
                raise RuntimeError("Invalid V20 USB sample-rate ACK")
        if self.offline_uv is None:
            self.set_sample_rate_local(rate)
        self.hardware_sample_rate = rate
        self.sample_rate_combo.setCurrentIndex(SAMPLE_RATES.index(rate))
        return rate

    def apply_sample_rate(self):
        try:
            rate = self.configure_sample_rate(int(self.sample_rate_combo.currentData()))
            self.set_status(f"V20 readback confirmed: {rate} SPS")
        except Exception as exc:
            self.sample_rate_combo.setCurrentIndex(SAMPLE_RATES.index(self.sample_rate))
            QtWidgets.QMessageBox.warning(self, "EMG sample rate", str(exc))

    def probe_serial_firmware(self):
        self.firmware_verified = False
        self.transport_reset_input_buffer()
        self.transport_write(bytes((0xAB,)))
        ack = self.read_config_ack(0xAB, expected_argument=3, timeout=1.0)
        packet = ack["packet"] if ack is not None else b""
        if (len(packet) != 12 or packet[3] != 20 or packet[6] != 1
                or not (int.from_bytes(packet[7:9], "little") & 0x40)
                or not ack["verified"]):
            raise RuntimeError("This product requires ESP32-C3 full-differential V20 / protocol 1")
        self.firmware_verified = True

    def sync_serial_configuration(self):
        """Set the host's complete channel state after USB identity verification."""
        enabled = sum(1 << ch for ch in range(CHANNELS) if self.channel_enabled[ch])
        bias = sum(1 << ch for ch in range(CHANNELS) if self.channel_enabled[ch] and self.channel_bias[ch])
        command = next(cmd for _name, cmd, mode in MODE_ITEMS if mode == self.current_mode)
        self.transport_write(command)
        self.transport_write(bytes((0xA5, 0, enabled, bias, 0, *map(int, self.channel_gains))))
        ack = self.read_config_ack(0xA5, expected_argument=enabled, timeout=2.0)
        expected_bias = bias if self.current_mode in (0, 1) else 0
        if (ack is None or not ack["verified"] or ack["enabled_mask"] != enabled
                or ack["bias_p"] != expected_bias or ack["bias_n"] != expected_bias
    ...
    def _main_range_changed(self, _viewbox, x_range):
        ...
        self.offline_end = min(self._total_samples(), int(x_range[1] * self.sample_rate))
        self.reset_psd_smoothing()
        self.update_fast_plots()
    ...
    def export_csv(self):
        ...
        matrix = np.column_stack((np.arange(self.offline_uv.shape[1]) / self.sample_rate, self.offline_uv.T))
        ...
        *,
        ...
        try:
            ...
            if remainder and not markers:
                annotations.append(
                    (
                        float(data.shape[1]) / rate,
                        float(rate - remainder) / rate,
                        "BDF_padding",
                    )
                )
    ...
    def export_recording_bdf(
        ...
        *,
        ...
        recording_rate = configuration.get("sample_rate_hz", self.sample_rate)
        sample_rate_code(recording_rate)
        ...
        gains = np.asarray(configuration.get("channel_gains", self.channel_gains), dtype=float)
        if gains.shape != (CHANNELS,) or not np.isin(gains, VALID_GAINS).all():
            raise ValueError("Invalid recorded channel gains")
        parser = AdsFrameParser(lambda: VREF / gains / (2**23 - 1) * 1e6)
        frames: list[Frame] = []
        ...
        (
            ...
            max_fill_samples=int(LIVE_TIMELINE_MAX_FILL_S * recording_rate),
        )
    ...
    def build_mne_raw(self):
        ...
        info = mne.create_info(
            channel_names, self.sample_rate, ch_types=[MNE_CHANNEL_TYPE] * CHANNELS
        )
        info["line_freq"] = 50.0
        info["description"] = (
        ...
        if valid.size == self.offline_uv.shape[1] and not valid.all():
            ...
            annotations = mne.Annotations(
                onset=starts.astype(float) / self.sample_rate,
                duration=(ends - starts).astype(float) / self.sample_rate,
    ...
    def save_mne_exports(self) -> Tuple[Path, Path]:
        ...
        time_s = np.arange(self.offline_uv.shape[1], dtype=np.float64) / self.sample_rate
    ...
    def toggle_connection(self):
        ...
        if self.selected_transport() == "ble":
            ...
            if not key:
                QtWidgets.QMessageBox.warning(self, "BLE", "请先扫描并选择 OmniBCI-C3-FULLDIFF-V20。")
                return
        ...
        try:
            ...
            self.sync_serial_configuration()
    ...
    def _ble_write_channel_config(self, ch: int, reference_mode: int):
        """V20 applies one atomic full-differential snapshot for every channel edit."""
    ...
    def _ble_write_bulk_config(self, reference_mode: int):
        ...
        if (
            ...
            or snapshot.bias_n != expected_bias_mask
            or snapshot.sample_rate_hz != self.sample_rate
    ...
    def sync_ble_configuration(self, requested_reference=None, probe_capability: bool = True):
        """V20 is fixed full differential; the reserved control byte stays zero."""
        self.set_reference_mode_local(REFERENCE_SRB1)
        return REFERENCE_SRB1, False

    def apply_ble_config_snapshot(self, snapshot):
        if self.offline_uv is None:
            self.set_sample_rate_local(snapshot.sample_rate_hz)
        self.hardware_sample_rate = snapshot.sample_rate_hz
    ...
    def on_ble_connected(self, name: str, address: str, mtu: int, reconnected: bool):
        ...
        if reconnected and self.streaming:
            if self.ble_worker is not None:
                self.ble_worker.set_streaming_hint(True)
            self.set_status(
                f"BLE 已自动重连并完成 V20/V1 握手：{name}，MTU={mtu}；"
                "正在继续原可靠会话并补传断线期间数据。"
            )
            return
        ...
        try:
            self.ble_supports_srb2 = False
            self.ble_reference_profile = "full_diff"
            ...
            self.set_status(
                ...
                f"固件 V{firmware[0]}.{firmware[1]}.{firmware[2]}，协议 V{protocol}，固定全差分。"
    ...
    def filter_offline_view(self, start: int, end: int) -> np.ndarray:
        ...
        context = max(3 * self.sample_rate, int(np.ceil(5.0 * self.sample_rate / hp)))
    ...
    def compute_live_psd_fast(
        ...
        display_f, display_p = signal.welch(
            display_signal, fs=self.sample_rate, window="hann", nperseg=nperseg,
    ...
    def compute_alpha_from_window(
        ...
        segment_len = self.sample_rate * 4
        step = self.sample_rate  # 75% overlap
    ...
    def open_impedance_dialog(self):
        ...
        note = QtWidgets.QLabel(
            ...
            "全差分同时激励 P/N；结果为待校准的回路估算值，检测期间不写入 BIN。"
        )
        note.setWordWrap(True)
    ...
    def start_impedance_detection(self):
        ...
        try:
            ...
            side = "INxP+INxN / LOFF_SENSP+LOFF_SENSN"
    ...
    def update_impedance_results(self):
        if not self.impedance_active or not self.impedance_value_labels:
            return
        values, valid, _seq, _mode = self.ring.latest(self.sample_rate * 4)
        if values.shape[1] < self.sample_rate:
            for ch in range(CHANNELS):
                if self.impedance_mask & (1 << ch):
                    self.impedance_value_labels[ch].setText("稳定中…")
            return
        t = np.arange(values.shape[1], dtype=float) / self.sample_rate
        ...
        for ch in range(CHANNELS):
            ...
            if np.count_nonzero(good) < self.sample_rate:
    ...
    def start_stream(self):
        if not self.require_transport():
            return
        if not self.firmware_verified or self.hardware_sample_rate is None:
            QtWidgets.QMessageBox.warning(self, "EMG V20", "请重新连接并确认 V20 全差分固件。")
            return
        if self.active_transport == "ble" and int(self.ble_status.get("status_protocol", 0)) != 0x05:
            QtWidgets.QMessageBox.warning(
                self,
                "BLE 固件不匹配",
                "请使用配套全差分 V20 固件 / STATUS V5。",
            )
            return
        if (
            self.active_transport == "ble"
    ...
    def poll_transport(self):
        ...
        try:
            ...
            if self.live_sample_count:
                self.live_lag_s = max(
                    float(self.last_pending) / self.sample_rate,
                    float(self.last_queue_depth) / self.sample_rate,
                    float(remaining) / (FRAME_BYTES * self.sample_rate),
                )
    ...
    def process_frames(self, frames: List[Frame], live: bool):
        ...
        if use_ble_timeline:
            (
                ...
                max_fill_samples=int(LIVE_TIMELINE_MAX_FILL_S * self.sample_rate),
            )
        ...
        for fr in frames:
            ...
            if fr.mode in (0, 1, 2):
                detected_reference = (
                    REFERENCE_SRB1  # V20 bit7=0 means SRB1 OFF, never SRB2
                )
        ...
        if live:
            ...
            self.live_lag_s = max(
                float(self.last_pending) / self.sample_rate,
                float(self.last_queue_depth) / self.sample_rate,
                float(self.last_serial_waiting_bytes) / (FRAME_BYTES * self.sample_rate),
            )
    ...
    def _finish_offline_load(self, path: str):
        ...
        self.offline_label.setText(
            f"{Path(path).name}: {self.offline_end / self.sample_rate:.1f}s"
        )
        if hasattr(self, "file_status"):
            self.file_status.setText(
                f"{Path(path).name}  |  {self.sample_rate} Hz  |  "
    ...
    def _load_bin_path(self, path: str):
        ...
        if sidecar.exists():
            ...
            sample_rate_code(rate)
            if "channel_gains" in configuration:
                gains = configuration["channel_gains"]
                if not isinstance(gains, list) or len(gains) != CHANNELS or any(
                    type(gain) is not int or gain not in VALID_GAINS for gain in gains
                ):
                    raise ValueError("Invalid recording channel gains")
            ...
            rate, accepted = QtWidgets.QInputDialog.getItem(
                ...
                SAMPLE_RATES.index(self.sample_rate), False)
            if not accepted:
                raise ValueError("BIN import cancelled: sample rate is required")
            rate = int(rate)
        ...
        if self.current_mode in (0, 1, 2):
            self.set_reference_mode_local(
                REFERENCE_SRB1  # Reserved zero for fixed full differential
            )
    ...
    def _load_bdf_path(self, path: str):
        ...
        if source_sfreq not in SAMPLE_RATES:
            raise ValueError("EMG BDF sample rate must be 250, 500 or 1000 SPS")
        ...
        self.set_sample_rate_local(int(source_sfreq))
        self.reset_processing_state()
    ...
    def import_file(self):
        ...
        try:
            ...
            if Path(path).suffix.lower() == ".bdf":
                ...
                resample_note = (
                    "" if np.isclose(source_sfreq, self.sample_rate)
                    else f"，已从 {source_sfreq:g} Hz 重采样到 {self.sample_rate} Hz"
                )
                pad_note = (
    ...
    def offline_slider_changed(self, value: int):
        ...
        self.reset_psd_smoothing()
        if self.offline_uv is not None:
            self.offline_label.setText(f"{self.offline_end/self.sample_rate:.1f}/{self.offline_uv.shape[1]/self.sample_rate:.1f}s")
        self.update_fast_plots()
    ...
    def update_adaptive_display_target(self):
        ...
        requested_samples = int(round(requested_s * self.sample_rate))
    ...
    def advance_live_display_cursor(self) -> int:
        ...
        if self.display_cursor_sample is None:
            ...
            self.display_delay_s = total / self.sample_rate
            if total < target:
                self.display_buffer_state = "priming"
                return 0
        ...
        reserve = max(0.0, float(total) - cursor)
        hard_max = int(round(DISPLAY_JITTER_HARD_MAX_S * self.sample_rate))
        hard_trigger = hard_max + int(round(DISPLAY_JITTER_HARD_HYSTERESIS_S * self.sample_rate))
        ...
        desired_advance = dt * self.sample_rate
        excess = max(0.0, reserve - float(target))
        catchup_trigger = DISPLAY_JITTER_CATCHUP_TRIGGER_S * self.sample_rate
        if excess > catchup_trigger and desired_advance > 0.0:
            ramp_span = max(1.0, 0.50 * self.sample_rate)
        ...
        self.display_delay_s = self.display_reserve_samples / self.sample_rate
        return end_sample
    ...
    def get_psd_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, int, int]:
        ...
        if self.offline_uv is not None:
            ...
            visible_start = int(round(float(self.start_time_spin.value()) * self.sample_rate))
            visible_length = max(1, int(round(float(self.win_spin.value()) * self.sample_rate)))
            ...
            analysis_length = min(total, max(10 * self.sample_rate, visible_end - visible_start))
    ...
    def update_legacy_fast_plots(self):
        ...
        t = (np.arange(y.size) - y.size + 1) / self.sample_rate
        ...
        if stack_data.shape[1] < 2:
            return
        stack_t = (np.arange(stack_data.shape[1]) - stack_data.shape[1] + 1) / self.sample_rate
        arr = np.vstack([
    ...
    def update_fast_plots(self, *_args):
        ...
        filter_backlog_s = self.filter_worker_backlog_samples() / self.sample_rate
        ...
        if is_live and self.active_transport == "ble":
            ...
            ble_backlog_s = ble_backlog / (FRAME_BYTES * self.sample_rate)
            ...
            if not self.display_buffer_started:
                if hasattr(self, "range_status"):
                    self.range_status.setText(
                        f"建立无线平滑缓冲：{self.display_reserve_samples/self.sample_rate:.2f}/"
                        f"{self.display_target_delay_samples/self.sample_rate:.2f} s；原始数据已正常保存"
                    )
                return
            if (
                self.sender() is getattr(self, "plot_timer", None)
    ...
    def _render_fast_plots(self, display_end_sample: Optional[int] = None):
        ...
        if self.offline_uv is not None:
            ...
            start_s = max(0.0, (display_end_sample - data.shape[1]) / self.sample_rate)
            self.start_time_spin.blockSignals(True)
        ...
        if t_rel is None:
            t_rel = np.arange(n_plot, dtype=np.float64) / self.sample_rate
        ...
        if self.offline_uv is not None:
            ...
            overview_t = np.arange(overview.size) * stride / self.sample_rate
    ...
    def update_psd_and_info(self):
        ...
        if data.shape[1] < self.sample_rate * 4:
            self.latest_window_good = False
    ...
    def offline_alpha_median_20s(self) -> Tuple[float, int, str]:
        ...
        if data.shape[1] < self.sample_rate * 8:
            return np.nan, 0, "数据不足 8 秒"
        ch = self.channel_combo.currentIndex()
        window = self.sample_rate * 4
    ...
    def update_info_text(self):
        ...
        entries = [
            ...
            ("Stale ctrl suppressed", f"N{reliable.get('stale_nack_suppressed', 0)} / A{reliable.get('stale_ack_suppressed', 0)}"),
    ...
    def make_verdict(self, saturation: float) -> str:
        if self.packet_count < self.sample_rate * 2 and self.offline_uv is None:
        ...
        if np.isfinite(self.fs_est) and abs(self.fs_est - self.sample_rate) > 2:
            return f"采样率 {self.fs_est:.1f} Hz 偏离 {self.sample_rate} Hz。"
        if saturation > 0.1:
    ...
    def closeEvent(self, event):  # noqa: N802
        ...
        for timer in self.findChildren(QtCore.QTimer):
            timer.stop()
        self.psd_pool.waitForDone()
        if self.soft_trigger_window is not None:
            self.soft_trigger_window.close()
        try:
            self.stop_stream()
            if self.stream_server is not None:
...
def main():
    ...
    APP_LOGGER.info("OmniEMG software=V20-integration.1 firmware=V20 protocol=V1 source=%s frozen=%s", __file__, IS_FROZEN)
    ...
    app.setApplicationName("OmniEMG Full Diff V20 integration.1")

    # Single-instance guard. A second instance would fight the first one for
    # the Windows BLE adapter, the serial port and the log file (double writes
    # were observed in onmibci.log), so show a notice and exit instead.
    # QLockFile is atomic at the OS level and auto-detects stale locks left by
    # a crashed process (default stale window: 30 s, keyed on PID + host).
    instance_lock = QtCore.QLockFile(str(LOG_DIR / "omniemg-v20.instance.lock"))
    if not instance_lock.tryLock(100):
        APP_LOGGER.warning("second instance rejected; lock=%s", instance_lock.fileName())
        QtWidgets.QMessageBox.warning(
            None,
            "OmniBCI",
            "程序已在运行，请勿重复启动。\n如程序刚刚异常退出，请稍等几秒后重试。",
        ...
        return 0
    # Keep the lock object alive until app.exec() returns; destruction releases it.
