...
def sample_rate_code(rate: int) -> int:
    if type(rate) is not int or rate not in SAMPLE_RATES:
        raise ProtocolError("sample rate must be 250, 500 or 1000 SPS")
...
def decode_config_snapshot(payload: bytes) -> ConfigSnapshot:
    payload = bytes(payload)
    if len(payload) != 29:
        raise ProtocolError(f"V20 config snapshot length {len(payload)} != 29")
    if payload[0] != 0:
        raise ProtocolError(f"device rejected request with result {payload[0]}")
    rate = int.from_bytes(payload[27:29], "little")
    code = sample_rate_code(rate)
    if payload[26] != code or payload[10] != 0x96 - code or payload[6] != 1:
        raise ProtocolError("sample rate / CONFIG1 readback is not verified")
    if payload[25] != 0 or any(value & 8 for value in payload[13:21]):
        raise ProtocolError("V20 full differential requires SRB1/SRB2 OFF")
    expected_bias = payload[8] & payload[7] if payload[5] in (0, 1) else 0
    if payload[21] != expected_bias or payload[22] != expected_bias:
        raise ProtocolError("V20 requires matching BIAS P/N readback")
