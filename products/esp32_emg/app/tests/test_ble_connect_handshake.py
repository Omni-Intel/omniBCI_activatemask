...
def _config_snapshot_payload(result: int = 0) -> bytes:
    """29-byte V20 GET_CONFIG response with full-differential registers."""
    ...
    ) + bytes([0x60] * 8) + bytes([0x1F, 0x1F, 0x00, 0x00, 0x00, 0, 250, 0])
...
class FakeBleClient:
    ...
    def _answer_device_packet(self, packet):
        if packet.message_type == MSG_HELLO:
            payload = bytes((0, 20, 0, 0, PROTOCOL_VERSION, 0x6F, 0, 1, 0, 0, 0))
