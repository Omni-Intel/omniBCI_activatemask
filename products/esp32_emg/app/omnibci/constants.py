"""Shared constants, device parameters and filesystem paths for OmniBCI."""
...
BLE_DEVICE_NAME_SRB1 = "OmniBCI-C3-FULLDIFF-V20"
...
# V19.0 firmware keeps only 16 blocks (~0.38 s) of unacked data in flight.
# Windows routinely batches DATA notifications and their ACK round-trips for
# several hundred milliseconds, which could fill that window and stall new
# transmissions (observed as ~20% sample delivery right before a link drop).
# Until the firmware widens the window, acknowledge more eagerly from the
# host: ACK every 2 blocks, at least every 50 ms, and run the repair
# watchdog at 20 Hz. Repeated identical cumulative ACKs are suppressed
# before the GATT write, so this adds no meaningful radio traffic.
BLE_RELIABLE_ACK_EVERY_BLOCKS = 2
BLE_RELIABLE_ACK_MAX_INTERVAL_S = 0.05
...
# 0.05 s (20 Hz) doubles repair/ACK-keepalive responsiveness to compensate the
# narrow 16-block firmware TX window of V19.0 (see BLE_RELIABLE_ACK_EVERY_BLOCKS).
BLE_RELIABLE_WATCHDOG_INTERVAL_S = 0.05
...
LEAD_OFF_FREQUENCY_HZ = 31.25  # LOFF=0x02 selects fCLK/65536, independent of CONFIG1 rate.
LEAD_OFF_CURRENT_NA = 6.0
LEAD_OFF_SERIES_SRB1_KOHM = 9.98
LEAD_OFF_SERIES_SRB2_KOHM = 4.40
REFERENCE_SRB1 = 0
REFERENCE_SRB2 = 1
REFERENCE_ITEMS = [("全差分 INxP-INxN（SRB1/SRB2 OFF）", REFERENCE_SRB1)]
MODE_ITEMS = [
    ("全差分 + BIAS P+N（兼容）", b"n", 0),
    ("全差分 + BIAS P+N", b"p", 1),
    ("全差分 + BIAS off", b"o", 2),
...
# This file lives in <root>/omnibci/, so the project root is one level up.
...
__all__ = [
    ...
    'SERIAL_MAX_PROCESS_FRAMES',
    'SERIAL_MAX_PROCESS_BYTES',
    'SERIAL_REPOLL_DELAY_MS',
    'BLE_COALESCE_MIN_FRAMES',
    'BLE_COALESCE_MIN_BYTES',
    'BLE_COALESCE_MAX_HOLD_S',
    'BLE_MAX_PROCESS_FRAMES',
    'BLE_MAX_PROCESS_BYTES',
    'BLE_PLOT_PAUSE_BACKLOG_BYTES',
    'BLE_PSD_PAUSE_BACKLOG_BYTES',
    'TRANSPORT_MAX_BATCH_FRAMES',
    'TRANSPORT_MAX_BATCH_BYTES',
    'TRANSPORT_NORMAL_BUDGET_S',
    'TRANSPORT_CATCHUP_BUDGET_S',
    'TRANSPORT_REPOLL_DELAY_MS',
    'TRANSPORT_CATCHUP_THRESHOLD_BYTES',
    'BLE_DEVICE_NAME_SRB1',
    'BLE_DEVICE_NAME_COMMON',
    'BLE_DEVICE_NAMES',
    'BLE_DEVICE_NAME',
    ...
    'BLE_RELIABLE_HOLE_RECONNECT_S',
    'BLE_RELIABLE_HOLE_TIMEOUT_S',
    ...
    'FILTER_OUTPUT_MAX_BATCHES',
    'FILTER_BACKLOG_PAUSE_PLOT_S',
    'FILTER_BACKLOG_PAUSE_PSD_S',
    'PSD_LIVE_REFRESH_MS',
    'PSD_LIVE_WINDOW_S',
    'ADC_SATURATION_FRACTION',
    'PSD_SATURATION_SKIP_RATIO',
    ...
    'DISPLAY_JITTER_LOW_RESERVE_RATE',
    'DISPLAY_JITTER_MAX_DT_S',
    'DISPLAY_JITTER_LONG_GAP_S',
    'DISPLAY_JITTER_CATCHUP_TRIGGER_S',
    'DISPLAY_JITTER_CATCHUP_MAX_RATE',
    'DISPLAY_JITTER_HARD_MAX_S',
    'DISPLAY_JITTER_HARD_HYSTERESIS_S',
    'LIVE_TIMELINE_MAX_FILL_S',
