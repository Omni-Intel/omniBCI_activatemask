...
class AsyncRawWriter:
    ...
    def __init__(self):
        ...
        self._segment_target_bytes = RECORD_SEGMENT_BYTES
    ...
    def start_session(self, folder: str, configuration: Optional[dict] = None):
        from onmibci_ble_protocol import sample_rate_code
        rate = (configuration or {}).get("sample_rate_hz", 250)
        sample_rate_code(rate)
        self.stop(timeout=2.0)
        if self._thread is not None and self._thread.is_alive():
            raise RuntimeError("上一次分包 BIN 写盘线程未退出，不能开始新的记录会话")
        self._bytes_per_second = FRAME_BYTES * rate
        self._segment_target_bytes = self._bytes_per_second * RECORD_SEGMENT_SECONDS
        target = Path(folder)
    ...
    def _update_current_record(self, status: str = "recording"):
        with self._lock:
            ...
            current["duration_seconds"] = round(
                self._segment_bytes / max(1, self._bytes_per_second), 3
            )
    ...
    def _write_payload(self, handle, payload: bytes):
        ...
        while offset < len(view):
            ...
            with self._lock:
                remaining = self._segment_target_bytes - self._segment_bytes
            ...
            with self._lock:
                ...
                full = self._segment_bytes >= self._segment_target_bytes
