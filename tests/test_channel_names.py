...
from onmibci_gui.window import MainWindow
...
class ChannelNameTests(unittest.TestCase):
    def make_window(self, window_type, transport="serial"):
        ...
        if window_type is MainWindow:
            window.validate_channel_names = MainWindow.validate_channel_names
    ...
    def test_name_only_never_touches_hardware_or_acquisition(self):
        for window_type in (legacy.MainWindow, MainWindow):
    ...
    def test_new_process_ignores_previously_saved_names(self):
        window = types.SimpleNamespace(app_settings=mock.Mock())
        window.app_settings.value.return_value = ["Fp1", *DEFAULT_CHANNELS[1:]]
        window.validate_channel_names = MainWindow.validate_channel_names
        self.assertEqual(MainWindow._load_channel_names(window), list(DEFAULT_CHANNELS))

    def test_bulk_naming_dialog_updates_api_without_persisting(self):
        # ponytail: isolate Qt lifetime from the other GUI/thread tests.
        result = subprocess.run(
            [sys.executable, "-c", "from tests.test_channel_names import ChannelNameTests; "
             "ChannelNameTests()._check_bulk_naming_dialog()"],
            cwd=Path(__file__).resolve().parents[1],
            capture_output=True, text=True, timeout=30,
        )
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)

    def _check_bulk_naming_dialog(self):
        class NamingWindow(QtWidgets.QWidget, ChannelConfigMixin):
            pass

        app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
        window = NamingWindow()
        window.channel_names = list(DEFAULT_CHANNELS)
        window.stream_server = LocalStreamServer(port=0)
        window.app_settings = mock.Mock()
        window.set_status = mock.Mock()

        def save_dialog():
            dialog = app.activeModalWidget()
            dialog.findChildren(QtWidgets.QLineEdit)[0].setText("Fp1")
            buttons = dialog.findChild(QtWidgets.QDialogButtonBox)
            buttons.button(QtWidgets.QDialogButtonBox.Save).click()

        try:
            QtCore.QTimer.singleShot(0, save_dialog)
            window.open_channel_naming_dialog()
            self.assertEqual(window.channel_names[0], "Fp1")
            self.assertEqual(window.stream_server._hello("raw")["channels"][0], "Fp1")
            window.app_settings.setValue.assert_not_called()
        finally:
