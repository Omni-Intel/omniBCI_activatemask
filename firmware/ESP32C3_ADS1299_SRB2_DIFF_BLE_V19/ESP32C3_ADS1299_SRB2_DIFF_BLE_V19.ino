/*
  ESP32-C3 + ADS1299：SRB2 EMG 差分固件 V19.1（软件 SPI）
  ...
  本固件目标是把采集链路状态完整、可验证地呈现出来：
    ...
    5. 固定使用 SRB2-INxN 差分输入，并提供短路噪声和内部方波诊断模式；
    6. 数据流中绝不插入状态文字，避免周期 impulse 和解析错位。

  默认上电配置：CH1-CH8 全开，每路 CHnSET.SRB2=1，MISC1.SRB1=0，
  BIAS 使用 N 侧：SENSP=0x00, SENSN=0xFF。
  ...
  串口命令：
    b : 开始二进制数据流
    s : 停止数据流
    e/p/m/* : EMG 推荐模式，SRB2，BIAS 取 N 端（CH1-8）
    n : EMG P/N BIAS 模式，SRB2，BIAS 同时取 P/N（CH1-8）
    o : EMG，SRB2，关闭 BIAS，用于判断 BIAS 环路是否引入问题
...
/*
  Fixed SRB2 differential-reference variant

  Wiring:
    EMG signal electrodes -> IN1N ... IN8N
    common reference      -> SRB2 / ASRB2
    bias electrode        -> BIASOUT

  Enforced configuration:
    - CHnSET.SRB2 is 1 for every enabled normal-input channel.
    - MISC1.SRB1 is always 0.
    - The default EMG mode uses BIAS_SENSN; BIAS_SENSP is 0.
*/
...
// V19 keeps configurationGeneration at byte 72 and appends acquisition-side
...
// GUI 可动态修改的逻辑 BIAS mask；本 SRB2 版实际写 BIAS_SENSN。
...
// bit2 is ignored because this build forces SRB2 on for every enabled normal channel.
...
void setup() {
  ...
  Serial.println("ESP32C3 ADS1299 SRB2 EMG FIRMWARE V19.1 / PROTOCOL V1 READY");
...
void setChannelConfig(uint8_t channel, uint8_t gain, uint8_t flags) {
  ...
  configureFrontend(static_cast<FrontendMode>(currentMode));
  if (!streamingEnabled) {
    Serial.printf("CH%u config: %s PGA=%ux BIAS_N=%u SRB1=OFF SRB2=1\n",
...
bool setBulkChannelConfig(const uint8_t *payload) {
  ...
  // Fixed-SRB2 firmware ignores host reference fields and performs one
  // complete write/readback cycle with SRB2 forced on.
  configureFrontend(static_cast<FrontendMode>(currentMode));
...
void printHelpAndDiagnostics() {
  Serial.println();
  Serial.println("=== ADS1299 SRB2 EMG diagnostic ===");
  ...
  Serial.printf("enabledMask=0x%02X biasLogicalMask=0x%02X SRB2=FORCED_ON\n",
                currentEnabledMask, currentBiasSensPMask);
  for (uint8_t ch = 0; ch < 8; ++ch) {
    Serial.printf("CH%u: %s PGA=%ux BIAS_N=%u SRB2=1\n",
